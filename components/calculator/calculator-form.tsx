...
import {
  PERSONAL_DEDUCTION_YEARLY,
  DEPENDENT_DEDUCTION_YEARLY,
  calculateTaxAnnualDirect,
  getAnnualBracketBreakdown,
  VANG_LAI_TAX_RATE,
} from "@/lib/tax-calculator";

// ✨ Mặc định: tính cho cả năm
// HH gross 360tr/năm = 30tr/tháng (giữ ý tưởng "1 tháng" cũ nhưng × 12)
const DEFAULT_VALUES = {
  hhGross: 360_000_000,
  salaryGross: 0,
  adsExpense: 60_000_000,
  deduction: PERSONAL_DEDUCTION_YEARLY, // 186.000.000 (cả năm)
};
...
export function CalculatorForm() {
  ...
  const calc = useMemo(() => {
    ...
    // ✨ Áp BẬC NĂM trực tiếp (không chia/nhân tháng)
    ...
    // Lợi nhuận = HH Gross − Tổng thuế − Chi phí Ads (lương không tính vào)
    const profit = hhGross - taxPayable - adsExpense;

    const base = hhGross > 0 ? hhGross : 1;
    const pct = (v: number) => ((v / base) * 100).toFixed(1) + "%";

    // ✨ Bracket breakdown theo NĂM
    const brackets = getAnnualBracketBreakdown(taxableIncome).map((b) => ({
      label: b.bracketRange,
      rate: b.rate,
    ...
    <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-base">Nhập thông số (cả năm)</CardTitle>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Mọi số liệu nhập theo NĂM để khớp với quyết toán thuế cuối năm
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            <FormRow label="Hoa hồng Gross (cả năm)">
              <CurrencyInput
                value={hhGross}
                onChange={setHhGross}
            ...
            <FormRow
              label="Lương (cả năm)"
              hint="Để 0 nếu không có. Chỉ dùng để tính thuế (không tính vào lợi nhuận)."
            >
              <CurrencyInput
                value={salaryGross}
                onChange={setSalaryGross}
                className="text-lg font-semibold tabular-nums"
              />
            </FormRow>

            <FormRow
              label="Giảm trừ gia cảnh (cả năm)"
              hint={`Bản thân: ${formatCurrency(PERSONAL_DEDUCTION_YEARLY)}/năm • Mỗi NPT: ${formatCurrency(DEPENDENT_DEDUCTION_YEARLY)}/năm`}
            >
              <CurrencyInput
                value={deduction}
              ...
              <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                ...
                <QuickButton
                  label="Bản thân"
                  value={PERSONAL_DEDUCTION_YEARLY}
                  current={deduction}
                  onClick={setDeduction}
                />
                <QuickButton
                  label="BT + 1 NPT"
                  value={PERSONAL_DEDUCTION_YEARLY + DEPENDENT_DEDUCTION_YEARLY}
                  current={deduction}
                  onClick={setDeduction}
                />
                <QuickButton
                  label="BT + 2 NPT"
                  value={PERSONAL_DEDUCTION_YEARLY + DEPENDENT_DEDUCTION_YEARLY * 2}
                  current={deduction}
                  onClick={setDeduction}
                />
                <QuickButton
                  label="BT + 3 NPT"
                  value={PERSONAL_DEDUCTION_YEARLY + DEPENDENT_DEDUCTION_YEARLY * 3}
                  current={deduction}
                  onClick={setDeduction}
                />
              </div>
            ...
            <FormRow label="Chi phí Facebook Ads (cả năm)">
              <CurrencyInput
                value={adsExpense}
                onChange={setAdsExpense}
                className="text-lg font-semibold tabular-nums"
              />
            </FormRow>
          </CardContent>
        ...
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground font-normal ml-2">
              Biểu lũy tiến 5 bậc theo NĂM - Luật 2026
            </span>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="grid gap-3 grid-cols-2 md:grid-cols-4 pb-4 border-b border-border">
            ...
            <StatBlock
              label="Đã khấu trừ 10%"
              value={`−${formatCurrency(calc.taxWithholding)}`}
              muted
            />
            <StatBlock
              label={
      ...
      <div className="flex items-start gap-3 p-4 rounded-md bg-muted/40 text-xs text-muted-foreground">
        ...
        <div className="space-y-1">
          <p className="font-medium text-foreground">Cách tính (cả năm)</p>
          <p>• <strong>Thuế tạm nộp</strong> = Hoa hồng Gross năm × 10% (Shopee khấu trừ tại nguồn)</p>
          <p>• <strong>Tổng thuế phải nộp</strong> = (HH Gross + Lương − Giảm trừ năm) áp biểu lũy tiến 5 bậc THEO NĂM</p>
          <p>• <strong>Bậc năm</strong>: Đến 120tr (5%) / 120-360tr (10%) / 360-720tr (20%) / 720tr-1.2 tỷ (30%) / Trên 1.2 tỷ (35%)</p>
          ...
          <p>• Đây là ước tính sơ bộ theo Luật Thuế TNCN 2025 (số 109/2025/QH15), cho quyết toán cả năm.</p>
        </div>
      </div>
    </div>
  );
...
// ============================================================================
// HELPERS (giữ nguyên y nguyên các helper cũ)
// ============================================================================
function FormRow({
  label,
  hint,
...
function BracketRow({
  ...
  const isReached = incomeInBracket > 0;
  return (
    <div
      className={cn(
        "grid grid-cols-12 items-center gap-2 px-3 py-2.5 rounded-md border border-border hover:bg-muted/40 transition-colors",
    ...
    >
      ...
      <div className="col-span-4 sm:col-span-3 text-right">
        ...
        <div className="text-sm tabular-nums text-muted-foreground">
          {isReached ? formatCurrency(incomeInBracket) : "—"}
        </div>
      </div>
      ...
      <div className="col-span-5 sm:col-span-2 text-right">
        ...
        <div className="text-sm font-bold tabular-nums">
          {isReached ? formatCurrency(taxInBracket) : "—"}
        </div>
      </div>
